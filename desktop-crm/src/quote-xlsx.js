...
function quoteSheetXml(input, options = {}) {
  ...
  const worksheet = `${XML_HEADER}<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView showGridLines="0" workbookViewId="0"/></sheetViews><sheetFormatPr defaultRowHeight="21"/><cols><col min="1" max="1" width="11" customWidth="1"/><col min="2" max="2" width="24" customWidth="1"/><col min="3" max="3" width="42" customWidth="1"/><col min="4" max="5" width="12" customWidth="1"/><col min="6" max="6" width="16" customWidth="1"/><col min="7" max="8" width="18" customWidth="1"/></cols><sheetData><row r="1" ht="42" customHeight="1">${inlineCell("A1", "견 적 서", 1)}</row><row r="2" ht="26" customHeight="1">${inlineCell("A2", quote.company.brand, 2)}${inlineCell("F2", "등록번호", 3)}${inlineCell("G2", quote.company.registrationNumber, 4)}</row><row r="3">${inlineCell("A3", "수신", 3)}${inlineCell("B3", `${quote.recipient} 귀중`, 4)}${inlineCell("F3", "상호", 3)}${inlineCell("G3", quote.company.businessName, 4)}</row><row r="4">${inlineCell("A4", "견적명", 3)}${inlineCell("B4", quote.projectName, 4)}${inlineCell("F4", "대표자", 3)}${inlineCell("G4", `${quote.company.representative} (인)`, 4)}</row><row r="5">${inlineCell("A5", "발행일", 3)}${inlineCell("B5", quote.quoteDate, 4)}${inlineCell("D5", "유효기간", 3)}${inlineCell("E5", quote.validUntil, 4)}</row><row r="6">${inlineCell("A6", "사업장 주소", 3)}${inlineCell("B6", quote.company.address, 4)}</row><row r="7">${inlineCell("A7", "전화", 3)}${inlineCell("B7", quote.company.phone, 4)}${inlineCell("D7", "팩스", 3)}${inlineCell("E7", quote.company.fax, 4)}</row><row r="8" ht="34" customHeight="1">${inlineCell("A8", "요약", 3)}${inlineCell("B8", quote.summary, 4)}</row><row r="9" ht="28" customHeight="1">${["No.", "품목", "상세 내용", "수량", "단위", "단가", "금액", "비고"].map((label, index) => inlineCell(`${String.fromCharCode(65 + index)}9`, label, 3)).join("")}</row>${rows}<row r="${supplyRow}">${inlineCell(`F${supplyRow}`, "공급가액", 7)}${numberCell(`G${supplyRow}`, quote.supplyAmount, 6)}</row><row r="${vatRow}">${inlineCell(`F${vatRow}`, "세액 (부가세 10%)", 7)}${numberCell(`G${vatRow}`, quote.vatAmount, 6)}</row><row r="${totalRow}" ht="30" customHeight="1">${inlineCell(`F${totalRow}`, "합계금액", 8)}${formulaCell(`G${totalRow}`, `SUM(G${firstItemRow}:G${lastItemRow})`, quote.totalAmount, 9)}</row><row r="${noteRow}" ht="64" customHeight="1">${inlineCell(`A${noteRow}`, "안내", 3)}${inlineCell(`B${noteRow}`, notes, 10)}</row><row r="${noteRow + 2}" ht="28" customHeight="1">${inlineCell(`A${noteRow + 2}`, quote.company.businessName || quote.company.name, 11)}</row></sheetData><mergeCells count="14"><mergeCell ref="A1:H1"/><mergeCell ref="A2:E2"/><mergeCell ref="G2:H2"/><mergeCell ref="B3:E3"/><mergeCell ref="G3:H3"/><mergeCell ref="B4:E4"/><mergeCell ref="G4:H4"/><mergeCell ref="B5:C5"/><mergeCell ref="E5:H5"/><mergeCell ref="B6:H6"/><mergeCell ref="B7:C7"/><mergeCell ref="E7:H7"/><mergeCell ref="B8:H8"/><mergeCell ref="B${noteRow}:H${noteRow}"/></mergeCells><pageMargins left="0.3" right="0.3" top="0.45" bottom="0.45" header="0.2" footer="0.2"/><pageSetup orientation="landscape" fitToWidth="1" fitToHeight="1" paperSize="9"/></worksheet>`;
  if (options.includeSeal !== true) return worksheet;
  return worksheet
    .replace('<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">', '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">')
...
function createQuoteWorkbook(input, now, sealInput) {
  const quote = QuoteCore.normalizeDraft(input);
  const createdAt = new Date(now || Date.now()).toISOString();
  const seal = Buffer.isBuffer(sealInput) ? sealInput : null;
  if (seal && (seal.length < 8 || seal.length > 750_000 || seal.subarray(0, 8).toString("hex") !== "89504e470d0a1a0a")) {
    throw new Error("견적서 인감 파일이 올바른 PNG가 아닙니다.");
  }
  const contentTypes = `${XML_HEADER}<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/>${seal ? '<Default Extension="png" ContentType="image/png"/><Override PartName="/xl/drawings/drawing1.xml" ContentType="application/vnd.openxmlformats-officedocument.drawing+xml"/>' : ""}<Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/><Override PartName="/docProps/core.xml" ContentType="application/vnd.openxmlformats-package.core-properties+xml"/><Override PartName="/docProps/app.xml" ContentType="application/vnd.openxmlformats-officedocument.extended-properties+xml"/></Types>`;
  const entries = {
    ...
    "xl/worksheets/sheet1.xml": quoteSheetXml(quote, { includeSeal: Boolean(seal) })
  };
  if (seal) Object.assign(entries, {
    "xl/worksheets/_rels/sheet1.xml.rels": `${XML_HEADER}<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" Target="../drawings/drawing1.xml"/></Relationships>`,
    "xl/drawings/drawing1.xml": `${XML_HEADER}<xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><xdr:oneCellAnchor><xdr:from><xdr:col>7</xdr:col><xdr:colOff>240000</xdr:colOff><xdr:row>3</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from><xdr:ext cx="540000" cy="540000"/><xdr:pic><xdr:nvPicPr><xdr:cNvPr id="1" name="BRING company seal"/><xdr:cNvPicPr><a:picLocks noChangeAspect="1"/></xdr:cNvPicPr></xdr:nvPicPr><xdr:blipFill><a:blip xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" r:embed="rId1"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr></xdr:pic><xdr:clientData/></xdr:oneCellAnchor></xdr:wsDr>`,
    "xl/drawings/_rels/drawing1.xml.rels": `${XML_HEADER}<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/bring-company-seal.png"/></Relationships>`,
    "xl/media/bring-company-seal.png": seal
  });
