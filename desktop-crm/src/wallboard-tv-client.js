...
const validUpdateStatus=value=>['idle','downloading','ready','installing','installed','failed'].includes(value);
function updateCommand(value){if(value===null||value===undefined)return null;if(!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).length!==1||!validVersion(value.targetVersion))throw new Error('업데이트 명령을 확인할 수 없습니다.');return {targetVersion:value.targetVersion};}
function createTvClient({vault,request,clientVersion}){
 const versionInfo=validVersion(clientVersion)?{clientVersion}:{};
 ...
 return {
  ...
  display:(updateState={status:'idle',error:''})=>serial(async()=>{const token=await vault.read();if(!validToken(token))return {paired:false};try{const body={...versionInfo};if(validUpdateStatus(updateState?.status))body.updateStatus=updateState.status;if(updateState?.error&&/^[A-Z0-9_]{1,80}$/.test(updateState.error))body.updateError=updateState.error;const result=await request('display',token,body),update=updateCommand(result.update);if(result.board===null)return {paired:true,board:null,update};const {version,publishedAt,...snapshot}=result.board||{};if(!Number.isSafeInteger(version)||version<1||!Number.isFinite(publishedAt))throw new Error('게시 자료를 확인할 수 없습니다.');return {paired:true,board:{...validatePublication(snapshot),version,publishedAt},update};}catch(error){if(['INVALID_TOKEN','AUTH_REQUIRED'].includes(error.code)){await vault.clear();return {paired:false,revoked:true};}throw error;}})
 };
}
