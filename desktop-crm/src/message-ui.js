(function (root, factory) {
  ...
  function renderDocumentDelivery(state) {
    ...
    const history = (state.documentDeliveries || []).map(item => `<tr><td>${esc(item.requestedAt || item.createdAt || "-")}</td><td>${esc(item.customerName || item.customerId || "-")}</td><td>${esc(item.documentName || "-")}</td><td>${esc(channelLabel(item.channel))}</td><td>${esc(item.status || "-")}</td></tr>`).join("");
