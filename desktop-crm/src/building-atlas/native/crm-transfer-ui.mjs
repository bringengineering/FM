...
export function setupCRMTransfer(api){
 ...
 $('fmBasicsFile').onchange=async e=>{pending=null;$('fmImportConfirm').disabled=true;$('fmImportPreview').textContent='';$('fmImportError').textContent='';try{const file=e.target.files[0];if(!file)return;if(file.size>10000)throw Error('10KB 이하의 건물정보 전용 파일을 선택해주세요.');const text=await file.text();document.assertActive();const value=JSON.parse(text),b=readTransfer(value);pending=value;$('fmImportPreview').textContent=`건물: ${b.name} · 주소: ${b.address||'미입력'} · 동일한 FM 건물은 기존 지도로 열립니다.`;$('fmImportConfirm').disabled=false;}catch(err){document.reportError('fmImportError',err);}};
 $('fmImportConfirm').onclick=async ()=>{if(!pending)return;try{const next=addTransferredBuilding(api.getPortfolio(),pending);if(next.items.length===api.getPortfolio().items.length)api.selectBuilding(next.activeId);else await api.setPortfolio(next);pending=null;dialog.close();api.toast('FM 건물을 열었습니다. 층수·치수·설비는 별도 확인해주세요.');}catch(err){document.reportError('fmImportError',err);}};
