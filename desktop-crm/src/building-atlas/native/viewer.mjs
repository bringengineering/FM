...
export function createViewer(host,onSelect){
 ...
 const scene=new T.Scene();scene.background=new T.Color(0xf2f6fc);
 ...
 function update(next,options){if(disposed)return;data=next;state=options;root.traverse(o=>{o.geometry?.dispose();if(o.material)for(const m of Array.isArray(o.material)?o.material:[o.material]){m.map?.dispose();m.dispose();}});scene.remove(root);root=new T.Group();scene.add(root);pickables=[];
 ...
 const outlineBox=new T.BoxGeometry(b.width,3,b.depth),outlineEdges=new T.EdgesGeometry(outlineBox);outlineBox.dispose();const outline=new T.LineSegments(outlineEdges,new T.LineBasicMaterial({color:0x52718e,transparent:true,opacity:.28}));outline.position.y=f*step+1.5;root.add(outline);}
 }
