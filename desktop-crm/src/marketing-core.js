(function (root, factory) {
  ...
  function buildAlerts(input, nowKst) {
    ...
    for (const fact of facts) {
      const entityType = bounded(fact.caseId) ? 'case' : bounded(fact.customerId) ? 'customer' : 'data', id = bounded(fact.caseId || fact.customerId) || 'aggregate-crm-facts', inquiryMs = instant(fact.inquiryAt || fact.occurredAt), nextMs = instant(fact.nextContactAt), quoteMs = instant(fact.quoteSentAt || fact.quotedAt), reviewMs = instant(fact.contractReviewAt || fact.updatedAt);
      const contractId = bounded(fact.contractId,160) || (Array.isArray(fact.contractIds) ? fact.contractIds : []).map(value=>bounded(value,160)).filter(Boolean).sort()[0] || '';
      const contractType = contractId ? 'contract' : entityType, contractTarget = contractId || id;
      const hasResponse = Boolean(fact.lastContactAt || fact.respondedAt || Number(fact.consultations) > 0);
      const quoteResponse = [fact.lastContactAt,fact.respondedAt,fact.responseAt,fact.consultedAt].map(instant).some(at=>at!=null&&quoteMs!=null&&at>quoteMs);
      if (inquiryMs != null && nowMs - inquiryMs >= .5 * HOUR && !hasResponse) add('inquiry_unanswered','urgent','30분 이상 미응답 문의','문의 후 연락·응답·상담 근거가 없습니다.',entityType,id,fact.inquiryAt || fact.occurredAt,new Date(inquiryMs + .5 * HOUR).toISOString(),false,{ inquiryAt:fact.inquiryAt || fact.occurredAt, consultations:Number(fact.consultations||0) });
      if (Number(fact.validLeads) > 0 && !bounded(fact.owner)) add('lead_missing_owner','warning','유효 리드 담당자 없음','유효 리드에 담당자가 지정되지 않았습니다.',entityType,id,fact.inquiryAt || fact.occurredAt,'',false,{ validLeads:Number(fact.validLeads) });
      if (Number(fact.validLeads) > 0 && !bounded(fact.nextContactAt)) add('missing_next_contact','warning','다음 연락일 없음','유효 리드의 다음 연락일이 없습니다.',entityType,id,fact.inquiryAt || fact.occurredAt,'',false,{ validLeads:Number(fact.validLeads), owner:fact.owner||'' });
      if (nextMs != null && kstDateKey(nextMs) === today) add('followup_today','info','오늘 후속 연락','다음 연락일이 오늘입니다.',entityType,id,fact.inquiryAt || '',fact.nextContactAt,false,{ nextContactAt:fact.nextContactAt });
      if (nextMs != null && kstDateKey(nextMs) < today) add('followup_overdue','urgent','후속 연락 기한 경과','다음 연락일이 지났습니다.',entityType,id,fact.inquiryAt || '',fact.nextContactAt,false,{ nextContactAt:fact.nextContactAt });
      if ((Number(fact.quotes)>0 || fact.analyticalStage === 'quote') && quoteMs != null && nowMs - quoteMs >= 24*HOUR && !quoteResponse) add('quote_no_response_24h','warning','견적 후 24시간 무응답','견적 발송 이후 응답·연락·상담 시각 근거가 없습니다.',entityType,id,fact.quoteSentAt || fact.quotedAt,new Date(quoteMs+24*HOUR).toISOString(),false,{ quotes:Number(fact.quotes||0), quoteId:fact.quoteId||'', quoteSentAt:fact.quoteSentAt||fact.quotedAt });
      ...
      if ((fact.customerStatus === 'lost' || fact.contractStatus === 'lost') && !bounded(fact.lostReason)) add('missing_lost_reason','warning','실패 이유 누락','실패 상태에 사유가 없습니다.',entityType,id,fact.occurredAt||'','',false,{ customerStatus:fact.customerStatus||'', contractStatus:fact.contractStatus||'' });
      ...
      if (['naver_place_ads'].includes(fact.channel) && Number(fact.inquiries||0)>0 && !bounded(fact.keyword)) add('missing_paid_keyword','info','유료검색 키워드 누락','유료검색 문의에 키워드가 없습니다.',entityType,id,fact.occurredAt||'','',false,{ channel:fact.channel });
    ...
    daily.slice().sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))||String(a.id||'').localeCompare(String(b.id||''))).forEach(row=>{ const budget=Number(row.dailyBudget),spend=Number(row.spend||0),validatedAt=Number(row.budgetValidatedAtMs); if(Number.isFinite(budget)&&budget>0&&Number.isSafeInteger(validatedAt)&&validatedAt>0&&spend/budget>=.8) add('budget_80_percent','warning','일예산 80% 이상 사용','검증된 해당 날짜 일예산 대비 지출이 80% 이상입니다.','ad',bounded(row.id,160)||'aggregate-daily-budget',row.date||'','',true,{date:row.date||'',spend,budget,usagePercent:Math.round(spend/budget*100)}); });
