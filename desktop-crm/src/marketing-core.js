(function (root, factory) {
  ...
  function buildAlerts(input, nowKst) {
    ...
    for (const fact of facts) {
      ...
      if ((Number(fact.quotes)>0 || fact.analyticalStage === 'quote') && quoteMs != null && nowMs - quoteMs >= 24*HOUR && !hasResponse && !fact.responseAt && !fact.nextStageAt) add('quote_no_response_24h','warning','견적 후 24시간 무응답','견적 발송 후 응답 또는 다음 단계 근거가 없습니다.',entityType,id,fact.quoteSentAt || fact.quotedAt,new Date(quoteMs+24*HOUR).toISOString(),false,{ quotes:Number(fact.quotes||0), quoteSentAt:fact.quoteSentAt||fact.quotedAt });
