(function (root, factory) {
  ...
  function render(model, options) {
    ...
    const dayCells = safeList(value.days).map(day => {
      ...
      const classes = ["work-calendar-day", day.inMonth ? "is-current-month" : "is-other-month", day.isToday ? "is-today" : "", day.isSelected ? "is-selected" : ""].filter(Boolean).join(" ");
      const eventMarkup = visible.map(event => renderEvent(event, canWrite, true)).join("");
      const labelParts = [`${dateLabel(day.date)}`, `${safeList(day.events).length}건`];
      return `<div role="gridcell" class="${classes}"${day.isSelected ? " aria-selected=\"true\"" : ""}><button type="button" class="work-calendar-day-number" data-work-calendar-date="${esc(day.date)}" aria-label="${esc(labelParts.join(", "))}" aria-pressed="${day.isSelected ? "true" : "false"}"${day.isToday ? " aria-current=\"date\"" : ""}><span>${esc(day.day)}</span>${day.isToday ? `<small>오늘</small>` : ""}</button><div class="work-calendar-day-events">${eventMarkup}${remainder ? `<span class="work-calendar-more">+ ${remainder}건</span>` : ""}</div></div>`;
    }).join("");
  ...
  return { buildModel, render, calendarDays, shiftMonth, monthLabel, dateLabel, typeLabel, statusLabel, isDateKey, isTimeKey };
});
