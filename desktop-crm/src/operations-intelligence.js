(() => {
  ...
  $("#operationForm").addEventListener("submit",async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),raw=Object.fromEntries(fd.entries());raw.interventionTypes=fd.getAll("interventionTypes");["siteVisit","remoteResolved","exceptionOccurred","replanned","firstTimeRight","revisitRequired","reworkRequired"].forEach(k=>raw[k]=fd.has(k));const result=await api.save(raw);if(!result.ok)return toast(result.error||"저장하지 못했습니다.");const i=state.operations.findIndex(x=>x.id===result.operation.id);if(i<0)state.operations.push(result.operation);else state.operations[i]=result.operation;$("#modal").hidden=true;render();toast("운영 기록을 서버에 저장했습니다.")});
