(function (root, factory) {
  ...
  function renderMarketingInput(options) {
    ...
    const valueFor = name => {
      ...
      return options.draft[name] || '';
    };
    return `<section class="marketing-input"><header><div><h2>광고 데이터 입력</h2><p>마지막 갱신 ${esc(options.lastUpdatedAt || "-")}</p></div>${options.canWrite && !options.loading && !options.saving ? `<button type="button" data-marketing-add>추가</button>` : ""}</header>${review}${options.canWrite && options.draft ? `<form data-marketing-entry-form>${fields.map(name => { const budget=name==='dailyBudget', validated=name==='budgetValidatedAt'; const label=budget?'검증된 일예산':validated?'예산 검증 시각':name; const type=validated?'datetime-local':['spend','impressions','clicks','phoneClicks','chatClicks','directionsClicks','saves','platformLeads','dailyBudget'].includes(name)?'number':''; return `<label><span>${esc(label)}</span><input name="${attr(name)}"${name === 'date' || name === 'channel' ? ' required' : ''}${type?` type="${type}"`:''}${type==='number'?' min="0" step="1"':''} value="${attr(valueFor(name))}">${validated?'<small>예산 근거 확인 후 입력하세요. 실제 검증 시각만 저장됩니다.</small>':''}</label>`; }).join('')}<button type="submit"${options.saving || options.loading ? ' disabled' : ''}>${options.saving ? '저장 중…' : '저장'}</button></form>` : ""}${content}</section>`;
  }
  ...
  function createEntryController(options) {
    ...
    function copy(row, date) {
      const draft = {};
      for (const name of ['channel','accountName','campaignId','campaignName','adGroup','keyword','contentId','contentTitle','service','region','spend','impressions','clicks','phoneClicks','chatClicks','directionsClicks','saves','platformLeads','dailyBudget','budgetValidatedAtMs','note']) if (row[name] != null) draft[name] = row[name];
      draft.date = date;
