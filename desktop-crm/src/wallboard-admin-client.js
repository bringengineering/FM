...
async function requestWallboardAdmin({baseUrl,idToken,input,fetchImpl=globalThis.fetch}){
 ...
 try{
  ...
  if(action==='list'){
   ...
   return {ok:true,version:data.version,devices:data.devices.map(d=>({id:String(d.id||''),name:String(d.name||'').slice(0,60),clientType:d.clientType==='web'?'web':'electron',createdAt:Number(d.createdAt)||0,lastSeenAt:timestamp(d.lastSeenAt),revokedAt:timestamp(d.revokedAt),clientVersion:semver(d.clientVersion),targetVersion:semver(d.targetVersion),updateStatus:statuses.has(d.updateStatus)?d.updateStatus:'idle',updateError:typeof d.updateError==='string'&&/^[A-Z0-9_]{1,60}$/.test(d.updateError)?d.updateError:null,updateApprovedAt:timestamp(d.updateApprovedAt),updateConsumedAt:timestamp(d.updateConsumedAt),updateCompletedAt:timestamp(d.updateCompletedAt)}))};
  }
