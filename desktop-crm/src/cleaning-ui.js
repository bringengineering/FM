(function (root, factory) {
  ...
  function renderCleaningCenter(input) {
    ...
    const inbound = active(options.inboundLeads).filter(item => ["new", "contact_required"].includes(item.status || "new"));
    const partnerApplications = inbound.filter(item => item.leadType === "partner_application");
    const inboundLeads = inbound.filter(item => item.leadType !== "partner_application");
    ...
    const messageOutbox = options.messageOutbox || {};
    const priorityMessages = active(messageOutbox.priorityMessages);
    ...
    return '<section class="cleaning-center" aria-labelledby="cleaningCenterTitle">' +
      ...
      '<section class="cleaning-partners"><header><div><span class="cleaning-eyebrow">INBOUND LEAD</span><h3>견적 신청함</h3><p>홈페이지에서 들어온 신규 문의 ' + inboundLeads.length + '건 · 확인 즉시 연락합니다.</p></div></header>' +
      (inboundLeads.length ? '<div class="cleaning-order-grid">' + inboundLeads.map(lead => '<article class="cleaning-order-card"><button type="button" data-cleaning-lead-convert="' + esc(lead.id) + '"' + (writable ? '' : ' disabled') + '><div><span>즉시 연락</span><strong>' + esc(lead.name || "고객명 미입력") + '</strong><small>' + esc(lead.phone || "연락처 미입력") + '</small></div><dl><div><dt>지역</dt><dd>' + esc(lead.location || "미입력") + '</dd></div><div><dt>서비스</dt><dd>' + esc(lead.service || "청소 견적") + '</dd></div><div><dt>접수</dt><dd>' + esc(lead.submittedAt || "방금") + '</dd></div></dl></button></article>').join("") + '</div>' : '<div class="cleaning-partner-empty">새로 접수된 홈페이지 견적 문의가 없습니다.</div>') + '</section>' +
      '<section class="cleaning-partners"><header><div><span class="cleaning-eyebrow">PARTNER RECRUITMENT</span><h3>Partner 지원 심사함</h3><p>신규 지원 ' + partnerApplications.length + '건 · 전화 인터뷰 후 유상 시험작업 2건으로 검증합니다.</p></div></header>' +
      (partnerApplications.length ? '<div class="cleaning-order-grid">' + partnerApplications.map(lead => '<article class="cleaning-order-card"><button type="button" data-cleaning-partner-lead="' + esc(lead.id) + '"' + (writable ? '' : ' disabled') + '><div><span>지원서 심사</span><strong>' + esc(lead.businessName || lead.name || "상호 미입력") + '</strong><small>' + esc(lead.phone || "연락처 미입력") + '</small></div><dl><div><dt>지역</dt><dd>' + esc(lead.location || "미입력") + '</dd></div><div><dt>인원</dt><dd>' + esc(lead.headcount || 1) + '명</dd></div><div><dt>서비스</dt><dd>' + esc(lead.services || "미입력") + '</dd></div></dl></button></article>').join("") + '</div>' : '<div class="cleaning-partner-empty">새로 접수된 Partner 지원서가 없습니다.</div>') + '</section>' +
      ...
      '<section class="cleaning-partners"><header><div><span class="cleaning-eyebrow">MESSAGE OUTBOX</span><h3>문자 발송대기함</h3><p>문구 검토 후 승인하며, 공급사 결과가 확인되기 전에는 발송완료로 표시하지 않습니다.</p></div></header><div class="cleaning-cash-kpis"><article><span>검토 초안</span><b>' + esc(messageOutbox.drafts || 0) + '건</b></article><article><span>공급사 전송대기</span><b>' + esc(messageOutbox.queued || 0) + '건</b></article><article><span>발송 실패</span><b>' + esc(messageOutbox.failed || 0) + '건</b></article><article><span>전달 증빙 완료</span><b>' + esc(messageOutbox.sent || 0) + '건</b></article></div>' +
      (priorityMessages.length ? '<div class="cleaning-case-list">' + priorityMessages.map(item => '<article><strong>' + esc(item.status === "failed" ? "발송 실패" : item.status === "queued" ? "공급사 전송대기" : "검토 초안") + ' · ' + esc(orderName(item.cleaningOrderId)) + '</strong><p>' + esc(item.recipient || "번호 미입력") + ' · ' + esc(item.body || item.templateId || "문구 미입력") + '</p>' + (item.failureReason ? '<p>실패 사유: ' + esc(item.failureReason) + '</p>' : '') + (writable && ["draft", "failed"].includes(item.status) ? '<button type="button" class="primary-button" data-cleaning-message-queue="' + esc(item.id) + '">' + (item.status === "failed" ? "재시도 대기" : "발송 승인") + '</button>' : '') + '</article>').join("") + '</div>' : '<div class="cleaning-partner-empty">처리할 고객 문자가 없습니다.</div>') + '</section>' +
  ...
  function renderCleaningOrderDetail(input) {
    ...
    return '<section class="cleaning-detail">' +
      ...
      (customerReports.length ? '<section class="cleaning-case-list"><h3>고객 완료보고서</h3>' + customerReports.map(item => '<article><strong>' + esc(item.status) + ' · QC ' + esc(item.qcScore) + '점</strong><p>완료사진 ' + active(item.photoUrls).length + '장 · ' + esc(item.completedAt || "완료시간 미입력") + '</p><button type="button" class="secondary-button" data-cleaning-customer-report-preview="' + esc(item.id) + '">출력·PDF</button>' + (options.writable !== false && item.status === "draft" ? '<button type="button" class="primary-button" data-cleaning-customer-report-deliver="' + esc(item.id) + '">고객 전달 확인</button>' : '') + '</article>').join("") + '</section>' : '') +
      (quotes.length ? '<section class="cleaning-case-list"><h3>고객 견적서</h3>' + quotes.map(item => '<article><strong>' + esc(item.status) + ' · ' + money(item.totalAmount) + '원</strong><p>' + esc(item.scope) + ' / 제외: ' + esc(item.exclusions || "없음") + '</p><p><strong>현장 추가금 없음</strong> · 유효기간 ' + esc(item.validUntil) + '</p><button type="button" class="secondary-button" data-cleaning-quote-preview="' + esc(item.id) + '">출력·PDF</button>' + (options.writable !== false && item.status === "draft" ? '<button type="button" class="primary-button" data-cleaning-quote-issue="' + esc(item.id) + '">고객 발행 확인</button>' : '') + '</article>').join("") + '</section>' : '') +
