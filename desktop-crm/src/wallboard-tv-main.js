...
const {createTvUpdateController}=require('./tv-update-controller');
...
let window,updateController;
app.whenReady().then(async()=>{
 const credential=path.join(app.getPath('userData'),'device.bin');
 const vault=createWindowsVault({credential,safeStorage});
 updateController=createTvUpdateController({updater:autoUpdater,currentVersion:app.getVersion(),resolveChannel:()=>resolveTvChannel()});
 ...
 handle('tv:start',()=>client.start());handle('tv:poll',()=>client.poll());handle('tv:display',async()=>{const result=await client.display(updateController.report()),{update,...safe}=result;if(update)void updateController.accept(update);return {...safe,updateState:updateController.report()};});
 handle('tv:fullscreen',()=>window.setFullScreen(!window.isFullScreen()));
 await window.loadFile(path.join(__dirname,'wallboard-tv.html'));
});
app.on('before-quit',()=>{updateController?.dispose();updateController=null;});
