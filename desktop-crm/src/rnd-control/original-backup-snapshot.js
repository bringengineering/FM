...
function createOriginalBackupSnapshotReader({access,captureSession,isCurrent,token,databaseUrl,fetch:request,allowEmptyRoot=false}){
 ...
 return async()=>{
  ...
  try{
   ...
   await check();const emptyRoot=value===null&&allowEmptyRoot===true;if(emptyRoot)value={};const etag=response.headers?.get('etag');if(typeof etag!=='string'||!etag||!value||typeof value!=='object'||Array.isArray(value))throw Error('백업 공유 자료·원격 버전 확인 실패');
   return{value,etag,emptyRoot,actorUid:actor.uid,contentSHA256:createHash('sha256').update(JSON.stringify(value)).digest('hex')};
