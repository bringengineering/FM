...
export function applyFollowUp(project,events,command,user,{at=new Date().toISOString()}={}){
 ...
 if(command.type==='create'){
  ...
  const allowed={todo:['active','blocked','cancelled'],active:['review','blocked','cancelled'],blocked:['active','cancelled'],review:['done','active','blocked','cancelled'],done:[],cancelled:[]};
  if(!allowed[previous.status].includes(command.status))throw Error('허용되지 않은 후속 업무 상태 전환');
 ...
 event={...event,id:command.eventId,actorUid:user.uid,reason:command.reason.trim(),at};const next=[...structuredClone(events),event];followUpTasks(next);return next;
}
