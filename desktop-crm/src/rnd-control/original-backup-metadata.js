...
async function prepareOriginalBackupSnapshot(input,selectedId){
 ...
 for(const project of projects)for(const dataset of project.research?.datasetSnapshots??[])if(!verifyDatasetManifest(dataset))throw Error('백업 동결 데이터셋 manifest 오류');
