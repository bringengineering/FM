...
function createOriginalRestoreReferenceExecution({access,captureSession,isCurrent,sessions,journal,drive,prepareResult}){
 return async input=>{
  ...
  await check();let prepared;const verification=await createOriginalRestoreReferenceVerifier({actorUid:actor.uid,check,prepare:async()=>prepared=await sessions.prepare(mappingRequest),journal,verifyVersion:ref=>drive.verifyVersion(ref)})(request);if(typeof prepareResult!=='function')return verification;await check();const preparation=await prepareResult({prepared,verification:structuredClone(verification),actor:structuredClone(actor)},{check});await check();return{...verification,preparation};
 };
}
