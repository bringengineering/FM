...
async function verifyOriginalBackup(bytes,{check}){
 ...
 await check();const rebuilt=await buildOriginalBackup(metadata,{check,download:async ref=>{const data=files.get('originals/'+ref.providerFileId+'.bin');if(!data)fail();return{reference:ref,bytes:data};}});
 ...
 const keys=['projectId','artifactId','versionId','providerFileId','sha256','sizeBytes','path'];
 for(let i=0;i<expected.files.length;i++)if(!manifest.files[i]||Object.keys(manifest.files[i]).length!==keys.length||keys.some(k=>manifest.files[i][k]!==expected.files[i][k]))fail();
