...
function createOriginalRestoreJournal(directory,{maxAttempts=100}={}){
 ...
 async function guard(target){const absolute=path.resolve(target),relative=path.relative(root,absolute);if(relative.startsWith('..')||path.isAbsolute(relative))throw Error('복원 원장 경로가 보관 폴더를 벗어났습니다');let current=path.parse(absolute).root;
  for(const part of absolute.slice(current.length).split(path.sep).filter(Boolean)){current=path.join(current,part);let info;try{info=await fs.lstat(current);}catch(error){if(error.code==='ENOENT')return;throw error;}if(info.isSymbolicLink())throw Error('복원 원장 연결 경로는 사용할 수 없습니다');const actual=await fs.realpath(current),equal=process.platform==='win32'?actual.toLowerCase()===current.toLowerCase():actual===current;if(!equal)throw Error('복원 원장 경로가 변경되었습니다');}
 }
