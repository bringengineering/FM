(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.BringCompanyWallboard=api;})(typeof globalThis==='object'?globalThis:this,function(){
 ...
 function scene(m,key,page=0,notice='',clock=new Date().toTimeString().slice(0,5)){
  ...
  if(key==='roadmap'){
   ...
   return `<div class="wb-roadmap-layout"><section class="wb-roadmap-board"><header><b>담당자 · 프로젝트</b>${weeks.map(week=>`<span>${esc(week.label)}</span>`).join('')}</header>${laneHtml||'<p class="wb-empty">등록된 프로젝트가 없습니다.</p>'}</section><aside class="wb-roadmap-summary"><div><span>전체 프로젝트 진행률</span><strong>${m.portfolio.overallProgress}%</strong><small>${m.portfolio.projects.length}개 프로젝트</small><progress max="100" value="${m.portfolio.overallProgress}"></progress></div><div class="wb-health-mini"><span>정상 <b>${h.normal}</b></span><span>확인 <b>${h.check}</b></span><span>위험 <b>${h.risk}</b></span><span>완료 <b>${h.done}</b></span></div><div><span>다가오는 마감</span>${m.portfolio.milestones.slice(0,3).map(item=>`<p><b>${item.daysLeft<0?'D+'+Math.abs(item.daysLeft):item.daysLeft===0?'D-DAY':'D-'+item.daysLeft}</b> ${esc(item.projectName)} · ${esc(item.owner)}</p>`).join('')||'<p>7일 안에 마감할 업무가 없습니다.</p>'}</div></aside></div>`;
  }
