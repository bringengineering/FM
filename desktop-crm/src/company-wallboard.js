(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.BringCompanyWallboard=api;})(typeof globalThis==='object'?globalThis:this,function(){
 ...
 function scene(m,key,page=0,notice='',clock=new Date().toTimeString().slice(0,5),zoom='week',dataDate=''){
  ...
  if(key==='weeklyTrend'){const max=Math.max(1,...m.portfolio.weeklyDone.map(item=>item.count));return `<div class="wb-weekly-chart">${m.portfolio.weeklyDone.map(item=>`<div><i style="height:${Math.max(5,item.count/max*100)}%"><b>${item.count}</b></i><span>${esc(item.label)}</span></div>`).join('')}</div><p>최근 5주간 검수 완료 업무 건수${m.portfolio.unattributedDone?` · 완료 시각 확인 필요 ${m.portfolio.unattributedDone}건`:''}</p>`;}
