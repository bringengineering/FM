(() => {
  ...
  const UNIFIED_CALENDAR_TABS = Object.freeze(["work", "contract", "payment"]);
  ...
  const viewMeta = {
    ...
    buildingCalendar: ["업무·계약·건물주 입금 일정을 한눈에", "캘린더"],
  ...
  function demoStore() {
    ...
    data.contracts = [
      ...
      Core.createContract({ id: "ctr_bukwon_grounds_20260815", type: "건물관리", name: "예초 작업", customerId: customers[0].id, buildingId: buildings[0].id, startDate: "2026-08-15", endDate: "2026-08-15", workDate: "2026-08-15", paymentDueDate: "2026-08-15", amount: 150000, vendorCost: 140000, billingCycle: "건별", collectionStatus: "입금 완료", vendorPaymentStatus: "지급 완료", status: "종료", owner: "서창환" }),
      Core.createContract({ id: "ctr_bukwon_waste_20260827", type: "청소", name: "예초집 폐기물 처리", customerId: customers[0].id, buildingId: buildings[0].id, startDate: "2026-08-27", endDate: "2026-08-27", workDate: "2026-08-27", paymentDueDate: "2026-08-27", amount: 35000, vendorCost: 32000, billingCycle: "건별", collectionStatus: "입금 예정", vendorPaymentStatus: "지급 완료", status: "진행 중", owner: "서창환" })
    ];
    data.activities = [
  ...
  function pageMeta() {
    ...
    if (calendarView) {
      calendarFolder?.classList.add("open");
      calendarFolder?.querySelector("[data-nav-folder-toggle]")?.setAttribute("aria-expanded", "true");
  ...
  const MANAGEMENT_FILTERS = Object.freeze(["전체", "연결 확인 필요", "관리 예정", "관리 중", "관리 종료"]);
  const MANAGEMENT_ORDER = Object.freeze({ "관리 중": 0, "관리 예정": 1, "연결 확인 필요": 2, "관리 종료": 3 });
  ...
  function managementStatusForCustomer(customer) {
    ...
    const buildings = customerBuildings(customer);
    if (!buildings.length) return "관리 예정";
  ...
  function renderCustomerHubDetail(customer) {
    ...
    const rentalDetails = managedBuilding ? (() => {
      const vacancySummary = vacancySummaryForBuilding(managedBuilding);
      const units = activeBuildingUnitsForBuilding(managedBuilding);
      const vacantUnits = vacancySummary.formal
        ? units.filter(item => vacancyUnitStatus(item) === "vacant").map(item => item.label || item.unitLabel).filter(Boolean)
        : buildingArray(managedBuilding.vacantUnits);
  ...
  function quoteSupplierFields() {
    const supplier = QuoteCore.companyProfile(aiAssistantState.supplier);
    if (aiAssistantState.supplierLoading) return `<section class="ai-quote-supplier-editor ai-quote-supplier-status"><b>회사 공급자 정보를 불러오는 중…</b></section>`;
    if (aiAssistantState.supplierConfigured && QuoteCore.supplierComplete(supplier)) {
      return `<section class="ai-quote-supplier-editor ai-quote-supplier-fixed"><header><div><b>공급자 정보</b><small>회사 공통 고정값 · AI 전송 안 함</small></div><span>고정</span></header><dl><div><dt>상호</dt><dd>${esc(supplier.businessName)}</dd></div><div><dt>대표자</dt><dd>${esc(supplier.representative)}</dd></div><div><dt>사업자등록번호</dt><dd>${esc(supplier.registrationNumber)}</dd></div><div class="wide address"><dt>소재지</dt><dd>${esc(supplier.address)}</dd></div><div class="wide"><dt>업태</dt><dd>${esc(supplier.businessType)}</dd></div><div class="wide"><dt>업종</dt><dd>${esc(supplier.businessCategory)}</dd></div></dl><div class="ai-quote-seal-control"><span>${aiAssistantState.sealConfigured ? "인감 보호 저장됨" : "인감 미등록"}</span>${aiAssistantState.sealCanConfigure ? `<button type="button" data-ai-quote-seal-select${aiAssistantState.sealLoading ? " disabled" : ""}>${aiAssistantState.sealConfigured ? "인감 변경" : "인감 등록"}</button>` : ""}</div><p>${aiAssistantState.sealError ? esc(aiAssistantState.sealError) : aiAssistantState.supplierCached ? "서버 연결 전 마지막 암호화 저장값을 사용 중입니다." : "인감 원본은 보호된 로컬 저장소에만 보관하고 견적서 안에서만 사용합니다."}</p></section>`;
    }
    if (!aiAssistantState.supplierCanConfigure) {
      return `<section class="ai-quote-supplier-editor ai-quote-supplier-status"><b>회사 공급자 정보가 아직 등록되지 않았습니다</b><p>${aiAssistantState.supplierError ? esc(aiAssistantState.supplierError) : "관리자 계정에서 최초 1회 등록해 주세요."}</p></section>`;
    }
    return `<section class="ai-quote-supplier-editor"><header><div><b>사업자등록번호 최초 등록</b><small>상호·대표자·주소·연락처는 BRING 고정값</small></div><button type="button" data-ai-quote-supplier-save${aiAssistantState.supplierSaving ? " disabled" : ""}>${aiAssistantState.supplierSaving ? "등록 중…" : "회사 공통 등록"}</button></header><dl class="ai-quote-supplier-preview"><div><dt>상호</dt><dd>${esc(supplier.businessName)}</dd></div><div><dt>대표자</dt><dd>${esc(supplier.representative)}</dd></div><div class="wide"><dt>사업장 주소</dt><dd>${esc(supplier.address)}</dd></div><div><dt>전화</dt><dd>${esc(supplier.phone)}</dd></div><div><dt>팩스</dt><dd>${esc(supplier.fax)}</dd></div></dl><div><label class="wide"><span>기존 사업자등록번호</span><input data-ai-quote-supplier="registrationNumber" maxlength="12" inputmode="numeric" autocomplete="off" value="${attr(supplier.registrationNumber)}" placeholder="000-00-00000"></label></div>${aiAssistantState.supplierError ? `<p class="ai-quote-supplier-error" role="alert">${esc(aiAssistantState.supplierError)}</p>` : `<p>스크린샷의 번호는 자동 입력하지 않습니다. 회사에 기존 저장된 번호를 확인해 등록합니다.</p>`}</section>`;
  }
  ...
  function applySupplierToQuote() {
    ...
    aiAssistantState.quote = QuoteCore.normalizeDraft(Object.assign({}, aiAssistantState.quote, {
      company: QuoteCore.companyProfile(aiAssistantState.supplier)
    }));
  }
  ...
  async function loadAiQuoteSupplier() {
    ...
    try {
      ...
      aiAssistantState.supplierConfigured = Boolean(result && result.configured);
      aiAssistantState.supplierCanConfigure = Boolean(result && result.canConfigure);
      aiAssistantState.supplierCached = Boolean(result && result.cached);
  ...
  async function saveAiQuoteSupplier() {
    ...
    try {
      ...
      aiAssistantState.supplierConfigured = true;
      aiAssistantState.supplierCanConfigure = Boolean(result && result.canConfigure);
      aiAssistantState.supplierCached = false;
      applySupplierToQuote();
      showToast("회사 공통 공급자 정보로 고정 등록했습니다.", "success");
    } catch (error) {
  ...
  async function loadAiQuoteSeal() {
    if (aiAssistantState.sealLoaded || aiAssistantState.sealLoading) return;
    aiAssistantState.sealLoading = true;
    aiAssistantState.sealError = "";
    try {
      const result = await api.loadQuoteSeal();
      aiAssistantState.sealDataUrl = result && result.dataUrl || "";
      aiAssistantState.sealConfigured = Boolean(result && result.configured && aiAssistantState.sealDataUrl);
      aiAssistantState.sealCanConfigure = Boolean(result && result.canConfigure);
      aiAssistantState.sealLoaded = true;
    } catch (error) {
  ...
  async function selectAiQuoteSeal() {
    ...
    try {
      ...
      aiAssistantState.sealDataUrl = result.dataUrl || "";
      aiAssistantState.sealConfigured = Boolean(result.configured && aiAssistantState.sealDataUrl);
      aiAssistantState.sealLoaded = true;
      showToast("인감을 보호 저장하고 견적서에 적용했습니다.", "success");
    } catch (error) {
      aiAssistantState.sealError = error.message || "인감을 등록하지 못했습니다.";
  ...
  function legacyQuotePreviewHtml(quote) {
  ...
  function loadQuoteSealImage() {
    if (!loadQuoteSealImage.promise) {
      loadQuoteSealImage.promise = new Promise((resolve, reject) => {
        const image = new Image();
        image.decoding = "async";
        image.onload = () => resolve(image);
        image.onerror = () => reject(new Error("견적서 인감 이미지를 불러오지 못했습니다."));
        image.src = "./assets/bring-company-seal.png";
      }).catch(error => {
        loadQuoteSealImage.promise = null;
        throw error;
      });
    }
    return loadQuoteSealImage.promise;
  }

  async function quoteImageDataUrl(quoteInput) {
    const quote = QuoteCore.normalizeDraft(quoteInput);
    const sealImage = await loadQuoteSealImage();
    ...
    const supplierRows = [["등록번호", quote.company.registrationNumber], ["상호", quote.company.businessName], ["대표자", quote.company.representative], ["사업장 주소", quote.company.address], ["전화 / 팩스", `${quote.company.phone} / ${quote.company.fax}`]];
    const supplierX = 714; const supplierY = 282; const supplierWidth = 422; const supplierRowHeight = 38;
    context.strokeStyle = "#b8cdd7"; context.lineWidth = 1; context.strokeRect(supplierX, supplierY, supplierWidth, supplierRowHeight * supplierRows.length);
    context.fillStyle = "#edf5f8"; context.fillRect(supplierX, supplierY, 98, supplierRowHeight * supplierRows.length);
    supplierRows.forEach((row, index) => {
      ...
      context.beginPath(); context.moveTo(supplierX + 98, rowY); context.lineTo(supplierX + 98, rowY + supplierRowHeight); context.stroke();
      context.fillStyle = "#587888"; context.font = "700 12px 'Malgun Gothic', sans-serif"; context.fillText(row[0], supplierX + 10, rowY + 24);
      context.fillStyle = "#173f56"; context.font = `${index >= 3 ? "700 11px" : "700 14px"} 'Malgun Gothic', sans-serif`; context.fillText(row[1] || "미입력", supplierX + 111, rowY + 24);
      if (index === 2) {
        context.save();
        context.globalAlpha = 0.88;
        context.translate(supplierX + supplierWidth - 32, rowY + 19);
        context.rotate(-7 * Math.PI / 180);
        context.drawImage(sealImage, -31, -31, 62, 62);
        context.restore();
      }
    });
    context.fillStyle = "#e9f7fc"; context.fillRect(104, 496, 1032, 124);
    context.fillStyle = "#267fa8"; context.font = "700 17px 'Malgun Gothic', sans-serif"; context.fillText("아래와 같이 견적합니다 · VAT 포함", 134, 540);
    context.fillStyle = "#173f56"; context.font = "900 43px 'Malgun Gothic', sans-serif"; context.fillText(QuoteCore.money(quote.totalAmount), 134, 589);
    context.fillStyle = "#527387"; context.font = "600 17px 'Malgun Gothic', sans-serif"; wrapCanvasText(context, quote.summary, 104, 662, 1032, 26, 2);
    ...
    y += 36;
    [["공급가액", quote.supplyAmount], ["세액 (부가세 10%)", quote.vatAmount], ["합계금액", quote.totalAmount]].forEach(([label, amount], index) => {
  ...
  async function exportAiQuote(copyType, format = "xlsx") {
    if (!aiAssistantState.quote) return;
  ...
  // 사이드바 숫자. 대표에게는 검수 대기, 담당자에게는 자기가 아직 못 끝낸
  // 것을 센다. 갱신하지 않으면 늘 0 으로 보이는데, 그건 "할 일 없음" 이라고
  // 거짓말하는 것과 같다.
  function updateWorkOrderBadge() {
    const badge = document.getElementById("navWorkOrderCount");
    if (!badge) return;
  ...
  function workOrderCard(W, order, today) {
    ...
    const late = W.overdue(order, today);
    const moves = (workOrderState.canWork && (workOrderState.admin || mine))
      ? W.nextChoices(order, workOrderState.admin)
      : [];
    const busy = workOrderState.busyId === order.id;

    const results = order.results.length
      ? `<ul class="wo-results">${order.results.map(item => `<li>
          <a href="${esc(item.webViewLink || "#")}" data-wo-open="${esc(item.webViewLink || "")}">${esc(item.title || "결과물")}</a>
          <small>${esc(item.uploadedBy)} · ${esc(String(item.uploadedAt).slice(0, 10))}</small>
        </li>`).join("")}</ul>`
  ...
  async function saveWorkOrderFromForm(form) {
    const W = workOrderCore();
    if (!W) return;
    const raw = Object.fromEntries(new FormData(form).entries());
  ...
  async function moveWorkOrder(orderId, next) {
    const W = workOrderCore();
    if (!W || workOrderState.busyId) return;
    let note = "";
    if (next === "returned") {
      note = String(window.prompt("다시 요청하는 이유를 적어 주세요.") || "").trim();
      if (!note) return showToast("이유를 적어야 합니다.", "error");
    }
    workOrderState.busyId = orderId;
    renderWorkOrders();
    try {
      await api.updateWorkOrderProgress({ id: orderId, status: next, note });
      workOrderState.loaded = false;
      showToast(`${W.statusLabel(next)} 로 옮겼습니다.`, "success");
      await loadWorkOrders();
    } catch (error) {
  ...
  // 결과물은 Drive 로 올라가고, 여기에는 링크만 남는다. 파일을 CRM 안에
  // 쌓으면 백업도 공유도 우리가 다시 만들어야 한다.
    ...
    try {
      for (const file of picked.files) {
        ...
        await api.updateWorkOrderProgress({
          id: orderId,
          result: {
            id: `res_${uploaded.driveFileId}`,
            title: uploaded.title || file.fileName,
            driveFileId: uploaded.driveFileId,
            webViewLink: uploaded.webViewLink || "",
            uploadedBy: (currentAuth.user && (currentAuth.user.displayName || currentAuth.user.email)) || "담당자",
      ...
      workOrderState.loaded = false;
      showToast(`결과물 ${done}건을 Drive 에 올렸습니다.`, "success");
      await loadWorkOrders();
    } catch (error) {
  ...
  function unifiedCalendarCounts(workModel) {
    const model = workModel || WorkCalendar.buildModel(store, {
    ...
    return {
      ...
      contract: Core.oneOffContractRows(store.contracts, contractCalendarMonth, "all").length,
      payment: paymentRows("all").length,
    };
  }
  ...
  function renderBuildingCalendar() {
    if (!new Set(["work", "contract"]).has(unifiedCalendarTab)) unifiedCalendarTab = "work";
  ...
  function renderPartnerVendorDetail(vendor) {
    const vendors = [...partnerVendorRows()].sort((left, right) => partnerVendorName(left).localeCompare(partnerVendorName(right), "ko"));
    const quotes = [...partnerQuotesForVendor(vendor)].sort((left, right) => String(right.consultedAt || right.receivedAt || right.contactedAt || right.createdAt).localeCompare(String(left.consultedAt || left.receivedAt || left.contactedAt || left.createdAt)));
    const activeQuotes = quotes.filter(item => item.status === "연락 예정" || item.status === "상담 중");
    const candidateQuotes = quotes.filter(item => item.status === "협력 후보");
    const recent = quotes[0] || null;
    const vendorOptions = vendors.map(item => `<option value="${attr(item.id)}" ${item.id === vendor.id ? "selected" : ""}>${esc([partnerVendorName(item) || "업체명 미입력", partnerIndustry(item), item.region].filter(Boolean).join(" · "))}</option>`).join("");
    const quoteRecord = item => {
      const consultedAt = item.consultedAt || item.receivedAt || item.contactedAt || item.createdAt;
      const price = moneyRange(item.totalMin || item.quotedAmount || item.constructionMin, item.totalMax || item.quotedAmount || item.constructionMax, item.constructionPricing || "가격 미확인");
      const editAttributes = canWriteCRM() ? ` data-partner-quote-edit="${attr(item.id)}" role="button" tabindex="0"` : "";
      return `<div class="building-detail-record customer-consultation-record${canWriteCRM() ? " clickable" : ""}"${editAttributes}><div><b>${esc(item.scenario || "문의한 작업 조건 미입력")}</b><span>${esc([item.status || "상태 미입력", consultedAt ? shortDate(consultedAt) : "상담일 미등록", item.consultationContent || item.memo].filter(Boolean).join(" · "))}</span></div><em>${esc(price)}</em></div>`;
    };
    const quoteRecords = quotes.map(quoteRecord).join("");
    const latestPrice = recent ? moneyRange(recent.totalMin || recent.quotedAmount || recent.constructionMin, recent.totalMax || recent.quotedAmount || recent.constructionMax, recent.constructionPricing || "가격 미확인") : "가격 미확인";
    main.innerHTML = `<section class="building-hub-hero customer-hub-hero partner-vendor-detail-hero"><div><span>업체를 선택하면 연락처와 상담 이력이 함께 열립니다</span><h2>협력 업체 정보를 한 화면에서 확인합니다</h2><p>고객·건물 관리와 같은 구조로 업체 기본정보와 상담 현황을 확인합니다.</p></div></section>
      <section class="customer-hub-workspace partner-vendor-detail-workspace"><header class="customer-hub-selector-bar"><div class="partner-vendor-detail-list-action"><button type="button" class="secondary-button" data-partner-vendor-detail-back>← 협력 업체 목록</button></div><label class="customer-select-control"><span>협력 업체</span><select data-partner-vendor-detail-select aria-label="협력 업체 선택">${vendorOptions}</select></label></header><section class="building-hub-detail"><header class="building-hub-detail-head customer-hub-detail-head"><div class="building-hub-title customer-hub-title"><span class="partner-vendor-detail-avatar large" aria-hidden="true">협</span><div><span>${esc(vendor.id)}</span><h2>${esc(partnerVendorName(vendor) || "업체명 미입력")}</h2><p>${esc([partnerIndustry(vendor), vendor.service || vendor.category || "작업 내용 미입력", vendor.region || "지역 미입력"].join(" · "))}</p></div></div><div class="building-hub-head-actions customer-hub-head-actions" role="group" aria-label="협력 업체 빠른 작업"><button type="button" class="secondary-button" data-partner-vendor-edit="${attr(vendor.id)}">업체 수정</button>${vendor.quoteUrl ? `<button type="button" class="primary-button" data-partner-vendor-link="${attr(vendor.quoteUrl)}">업체 페이지 열기 ↗</button>` : ""}</div></header><div class="building-hub-detail-scroll"><div class="building-identity-strip customer-essential-summary"><div><b>대표 연락처</b><span>${esc(vendor.phone ? `${vendor.phoneLabel ? `${vendor.phoneLabel} ` : ""}${customerPhoneText(vendor.phone)}` : "미입력")}</span></div><div><b>보조 연락처</b><span>${esc(vendor.alternatePhone ? `${vendor.alternatePhoneLabel ? `${vendor.alternatePhoneLabel} ` : ""}${customerPhoneText(vendor.alternatePhone)}` : "미입력")}</span></div><div><b>지역</b><span>${esc(vendor.region || "미입력")}</span></div><div><b>업종</b><span>${esc(partnerIndustry(vendor))}</span></div></div><div class="building-hub-kpis customer-hub-kpis"><div class="building-hub-kpi"><span>전체 상담</span><b>${quotes.length}건</b><small>${recent ? `최근 ${esc(shortDate(recent.consultedAt || recent.receivedAt || recent.contactedAt || recent.createdAt))}` : "상담 기록 없음"}</small></div><div class="building-hub-kpi ${activeQuotes.length ? "alert" : ""}"><span>진행 상담</span><b>${activeQuotes.length}건</b><small>${esc(activeQuotes[0]?.scenario || "진행 중인 상담 없음")}</small></div><div class="building-hub-kpi"><span>협력 후보</span><b>${candidateQuotes.length}건</b><small>${esc(latestPrice)}</small></div></div><div class="building-detail-grid customer-priority-grid"><section class="building-detail-section"><header><b>업체 기본 정보</b><span>등록 내용</span></header><div class="building-detail-body"><div class="building-detail-record"><div><b>${esc(vendor.service || vendor.category || "작업 내용 미입력")}</b><span>${esc(vendor.memo || "등록된 업체 메모가 없습니다.")}</span></div></div></div></section><section class="building-detail-section"><header><b>연락 정보</b><span>${vendor.quoteUrl ? "업체 링크 등록" : "업체 링크 미등록"}</span></header><div class="building-detail-body"><div class="building-detail-record"><div><b>${esc(partnerPhoneText(vendor))}</b><span>${esc([vendor.region || "지역 미입력", vendor.quoteUrl ? "업체 페이지 연결됨" : "업체 페이지 미등록"].join(" · "))}</span></div></div></div></section><section class="building-detail-section wide"><header><b>최근 상담 기록</b><span>${quotes.length}건</span></header><div class="building-detail-body">${quoteRecords || `<div class="building-detail-empty">아직 등록된 업체 상담 기록이 없습니다.</div>`}</div></section></div></div></section></section>`;
  ...
  function showPartnerVendorDetailAfterQuoteMutation(vendorId) {
    selectedPartnerVendorDetailId = vendorId || "";
    currentView = "partnerVendors";
    render();
    const consultationHistory = main.querySelector("[data-partner-vendor-consultations]");
  ...
  async function loadTelegramSettings() {
    ...
    try {
      ...
      telegramState = Object.assign({}, telegramState, {
        ...
        hour: Number(result && result.hour) >= 0 ? Number(result.hour) : 9,
  ...
  function telegramCard() {
    ...
    return `<section class="setting-card tg-card">
      ...
      <form data-telegram-form>
        ...
        <label class="field tg-hour"><span>보낼 시각</span><select name="hour">${Array.from({ length: 24 }, (_unused, hour) => `<option value="${hour}"${hour === telegramState.hour ? " selected" : ""}>${hour}시</option>`).join("")}</select></label>
  ...
  async function maybeAutoSendTelegram() {
    ...
    const verdict = T.dueNow({
      ...
      hour: telegramState.hour,
      lastAutoDay: telegramState.lastSentDay,
    }, new Date());
  ...
  setInterval(() => {
    if (telegramAutoTriedDay && telegramAutoTriedDay !== todayKey()) telegramAutoTriedDay = "";
    void maybeAutoSendTelegram();
  }, 15 * 60 * 1000);
  ...
  function partnerQuoteEditor(quoteId, vendorId = "", returnView = currentView) {
    ...
    const requestedVendor = partnerVendorById(vendorId);
    const linkedVendor = editing ? partnerVendorForQuote(item) : requestedVendor || partnerVendorForQuote(item);
    item.vendorId = linkedVendor && linkedVendor.id || "";
    item.industry = editing ? partnerIndustry(item) : linkedVendor ? partnerIndustry(linkedVendor) : "";
    ...
    form.dataset.returnView = returnView || "partnerQuotes";
    form.dataset.returnVendorId = linkedVendor && linkedVendor.id || vendorId || "";
  ...
  async function deletePartnerQuoteRecord(quoteId, returnView = currentView, returnVendorId = "") {
    ...
    scheduleSave();
    closeModal();
    if (returnView === "partnerVendors") {
      showPartnerVendorDetailAfterQuoteMutation(returnVendorId || quote.vendorId);
    } else {
  ...
  document.addEventListener("click", async event => {
    ...
    if (woOpen) {
      // Drive 링크는 기본 브라우저로 연다. 앱 안에서 열면 로그인이 또 필요하다.
      event.preventDefault();
      const link = woOpen.dataset.woOpen;
      if (link) await api.openExternal(link);
      return;
    }
    ...
    if (aiQuoteExport && !aiQuoteExport.disabled) { await exportAiQuote(aiQuoteExport.dataset.aiQuoteExport, aiQuoteExport.dataset.aiQuoteFormat || "xlsx"); return; }
    ...
    if (partnerQuoteDelete) {
      const form = partnerQuoteDelete.closest("#partnerQuoteForm");
      await deletePartnerQuoteRecord(partnerQuoteDelete.dataset.partnerQuoteDelete, form?.dataset.returnView, form?.dataset.returnVendorId);
      return;
    }
    ...
    const partnerVendorOpen = event.target.closest("[data-partner-vendor-open]");
    if (partnerVendorOpen) {
      selectedPartnerVendorDetailId = partnerVendorOpen.dataset.partnerVendorOpen || "";
      renderPartnerVendors();
      pageMeta();
      return;
    }
  ...
  document.addEventListener("change", async event => {
    ...
    if (event.target.matches("[data-ai-quote-recipient]")) {
      ...
      if (!quote || !["projectName", "siteAddress", "recipient", "recipientPhone", "quoteDate", "validUntil"].includes(key)) return;
  ...
  document.addEventListener("submit", async event => {
    ...
    if (form.id === "buildingScheduleForm") {
      ...
      await refreshOperations({ silent: true, render: false });
      closeModal(); currentView = "payments"; render(); showToast("납부 일정을 건물주 입금캘린더에 저장했습니다.", "success");
      ...
      await refreshOperations({ silent: true, render: false });
      closeModal(); currentView = "payments"; render(); showToast("입금 상태를 건물주 입금캘린더에 반영했습니다.", "success");
      ...
      try {
        ...
        const returnView = ["customers", "vacancies", "payments"].includes(form.dataset.returnView) ? form.dataset.returnView : "buildings";
        if (returnView === "vacancies") selectedVacancyBuildingId = selectedBuildingId;
        if (returnView === "payments") paymentBuildingFilter = selectedBuildingId;
      ...
      const beforeStore = cloneStore(store);
      const returnView = form.dataset.returnView || "partnerQuotes";
      const raw = Object.fromEntries(new FormData(form).entries());
  ...
  document.addEventListener("input", event => {
    ...
    if (event.target.matches("[data-customer-building-picker-search]")) {
      ...
      aiAssistantState.supplierError = "";
      return;
    } else if (event.target.matches("[data-ai-quote-recipient]")) {
      const quote = aiAssistantState.quote;
      const key = event.target.dataset.aiQuoteRecipient;
      if (!quote || !["projectName", "siteAddress", "recipient", "recipientPhone", "quoteDate", "validUntil"].includes(key)) return;
      const maxLength = key === "projectName" ? 120 : key === "siteAddress" ? 180 : key === "recipient" ? 80 : key === "recipientPhone" ? 30 : 10;
      quote[key] = event.target.value.slice(0, maxLength);
      aiAssistantState.quoteError = "";
      return;
...
document.addEventListener("keydown", event => {
  ...
  if (unifiedCalendarTabControl && ["ArrowRight", "ArrowLeft", "Home", "End"].includes(event.key)) {
    event.preventDefault();
    const currentTab = UNIFIED_CALENDAR_TABS.includes(unifiedCalendarTabControl.dataset.unifiedCalendarTab) ? unifiedCalendarTabControl.dataset.unifiedCalendarTab : "work";
    const currentIndex = UNIFIED_CALENDAR_TABS.indexOf(currentTab);
    const nextTab = event.key === "Home" ? UNIFIED_CALENDAR_TABS[0]
      : event.key === "End" ? UNIFIED_CALENDAR_TABS[UNIFIED_CALENDAR_TABS.length - 1]
      : UNIFIED_CALENDAR_TABS[(currentIndex + (event.key === "ArrowRight" ? 1 : -1) + UNIFIED_CALENDAR_TABS.length) % UNIFIED_CALENDAR_TABS.length];
    unifiedCalendarTab = nextTab;
    currentView = nextTab === "payment" ? "payments" : "buildingCalendar";
    render();
    main.scrollTop = 0;
