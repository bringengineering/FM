(function (root, factory) {
  ...
  function renderDashboard(model, options) {
    ...
    const cards = items.map(item => {
      ...
      return `<article class="work-card" data-work-id="${esc(item.id)}"><div class="work-card-status status-${esc(item.status)}">${esc(statusLabel(item.status))}</div><div class="work-card-main"><span>${esc(item.building && (item.building.name || item.building.address) || "건물 미연결")}</span><h3>${esc(item.title)}</h3><p>${esc([date, cadence, item.vendorName, cost].filter(Boolean).join(" · "))}</p><small>${esc(item.summary || (item.kind === "contract" ? "정기 작업 계약" : "작업 내용 미입력"))}</small>${sync ? `<small class="work-sync-state sync-${esc(item.operationsSyncStatus)}">${esc(sync)}</small>` : ""}</div><div class="work-card-actions">${item.evidenceUrl ? `<button class="secondary-button" data-work-evidence="${esc(item.evidenceUrl)}">Drive 증빙 ↗</button>` : ""}${retry}${opts.canWrite ? `<button class="mini-button" data-work-edit="${esc(item.id)}">상세</button>` : ""}</div></article>`;
    }).join("");
