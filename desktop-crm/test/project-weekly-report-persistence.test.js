...
function client(role = 'member', uid = 'u1', existing = null) {
  ...
  remote.dbReadWithEtag = async () => ({ value: existing, etag: 'etag-1' });
...
test('승인본 정정은 원본을 유지하고 새 ID에 이전 승인본 링크를 남긴다', async () => {
  ...
  const remote = client();
  remote.dbRequest = async location => location === 'workOrders' ? sourceOrders : location === 'projectWeeklyReports/r1' ? original : null;
...
test('서버 규칙은 승인본 수정·삭제와 다른 사람의 보고서 수정을 막는다', () => {
  ...
  assert.match(write, /data\.child\('status'\)\.val\(\) !== 'approved'/u);
