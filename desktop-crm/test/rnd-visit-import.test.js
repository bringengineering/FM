const test=require('node:test'),assert=require('node:assert/strict');test('baseline import preserves existing IDs and creates revision-zero drafts with original revision retained separately',async()=>{const {previewVisitImport}=await import('../src/rnd-control/visit-import.mjs');const v={id:'v',projectId:'p',revision:5,buildingId:'b',operator:'o',reason:'r',serviceScope:'s',costBasis:'c',evidenceUrl:'https://example.com',date:'2026-09-17',minutes:{travel:0,check:0,work:0,report:0,contact:0,other:0},costs:{labor:0,transport:0,materials:0,outsourcing:0},totalMinutes:0};const plan=previewVisitImport([{id:'existing'}],[v,{...v,id:'existing'}],['p']);assert.deepEqual(plan.conflicts,['existing']);assert.equal(plan.additions[0].data.revision,0);assert.equal(plan.additions[0].sourceRevision,5);assert.equal(v.revision,5);assert.equal(plan.additions[0].data.restorationSourceRevision,5);const reimport=previewVisitImport([],JSON.parse(JSON.stringify([plan.additions[0].data])),['p']);assert.equal(reimport.additions[0].sourceRevision,5);assert.throws(()=>previewVisitImport([],[{...v,restorationSourceRevision:'bad'}],['p']),/복원/);assert.throws(()=>previewVisitImport([],[v],[]),/프로젝트/);});
