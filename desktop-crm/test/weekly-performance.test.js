...
test('renderWorkOrders preserves raw records through mine/all performance scope',()=>{
 ...
 let captured;
 const ctx={window:{},main:{innerHTML:''},companyStrategyState:{loaded:true,error:'unavailable'},renderCompanyStrategy:()=>'',workOrderState:{orders,performanceOrders:orders,projects:[{id:'p',name:'가상',status:'active'}],scope:'mine',uid:'u'},workOrderCore:()=>require('../src/work-order-core'),projectCore:()=>require('../src/project-core'),todayKey:()=>'2026-09-14',esc:String,refreshButton:()=>'',weeklyExecutionPanel:()=>'',weeklyPerformancePanel:rows=>{captured=core().summarize({orders:rows,asOf:'2026-09-14'});return '';}};
