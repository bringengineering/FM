...
test('original reupload Main IPC binds starting SDK and denies local company writes before creating execution adapters',async()=>{const line=fs.readFileSync(path.join(__dirname,'../src/main.js'),'utf8').split('\n').find(line=>line.startsWith("secureHandle('crm:rnd-upload-original-restore',"));assert.ok(line);for(const change of [true,false]){let g=1,handler,calls=0;vm.runInNewContext(line,{secureHandle:(_name,fn)=>handler=fn,csvSessionBinding:()=>g,csvSessionCurrent:b=>b===g,assertMainMutationAllowed:()=>{},assertRndAccess:async()=>{if(change)g++;},localTestMode:true,require:()=>{calls++;throw Error('no writes');},originalRestoreUploadBusy:false});await assert.rejects(()=>handler({}),change?/세션/:/로컬 시험/);assert.equal(calls,0);}assert.match(fs.readFileSync(path.join(__dirname,'../src/preload.js'),'utf8'),/rndUploadOriginalRestore:.*crm:rnd-upload-original-restore/);});
