...
type Root=Record<string,unknown>;interface Actor {uid:string;email:string;}
export interface RestoreApprovalSession {id:string;actorUid:string;actorEmail:string;state:'PENDING'|'COMMITTED';createdAt:number;expiresAt:number;committedAt?:number;expectedSnapshotSHA256:string;mutation:RestoreMutation;}
...
export function createRestoreApprovalSession({actor,mutation,expectedSnapshotSHA256,now}:{actor:Actor;mutation:RestoreMutation;expectedSnapshotSHA256:string;now:number}):RestoreApprovalSession {
 if(!Number.isSafeInteger(now)||now<0||!Number.isSafeInteger(now+600000)||!actor.uid||!actor.email||mutation.audit.actorUid!==actor.uid)throw Error('Invalid restore approval session');
 return{id:mutation.operationId,actorUid:actor.uid,actorEmail:actor.email,state:'PENDING',createdAt:now,expiresAt:now+600000,expectedSnapshotSHA256,mutation:structuredClone(mutation)};
}
...
export function createRestoreSessionUpdater(input:{actor:Actor;sessionId:string;digest:(value:unknown)=>string;now:()=>number}) {
 ...
 return(current:unknown):Root|undefined=>{
  if(!record(current))return undefined;const sessions=own(current,'rndRestoreSessions'),owner=own(sessions,actor.uid),stored=own(owner,id);if(!record(stored)||stored.id!==id||stored.actorUid!==actor.uid||stored.actorEmail!==actor.email)return undefined;
