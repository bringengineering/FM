...
import {createRestoreApprovalSession,createRestoreSessionUpdater,createRestoreSessionStoreUpdater,createRestoreSessionRootStoreUpdater} from './rnd/restore-session.js';
...
export const rndPrepareSharedRestore = onCall({region:'asia-southeast1',timeoutSeconds:120,memory:'1GiB'},async request=>{
 ...
 const stored=await runRestoreRootTransaction(adminDatabase.ref(),createRestoreSessionRootStoreUpdater({actor,session,digest:runtime.digest,now:Date.now}));if(!stored.committed)throw new HttpsError('aborted','승인 요청을 보관하지 못했습니다');return{sessionId:session.id,expiresAt:session.expiresAt,projects:mutation.audit.projects,visits:mutation.audit.visits,scope:'metadata-only'};
});
