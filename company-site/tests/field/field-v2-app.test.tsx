...
describe("FIELD v2 app gateway", () => {
  ...
  it("approves zero pending uploads only for an authoritative no-session state", async () => {
    ...
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(post.mock.calls.map(([message]) => message).filter((message: any) => message.type === "field.logoutDecision"))
      .toHaveLength(0);
  ...
  it("does not approve logout while an authenticated runtime has unknown uploads", async () => {
    const post = vi.spyOn(window, "postMessage");
