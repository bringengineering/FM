...
test('background refresh retains pagination so every employee can be displayed',async()=>{
 ...
 for(let i=0;i<20&&host.querySelector('h1')?.textContent!=='사람별 업무';i++)host.querySelector('[data-wb="next"]').click();
 expect(host.querySelector('h1').textContent).toBe('사람별 업무');host.querySelector('[data-wb="next"]').click();expect(host.querySelector('.wb-content').textContent).toContain('2/3페이지');
...
test('local playlist persists only controls and supports an empty playlist',async()=>{
 ...
 await new Promise(r=>setTimeout(r,0));
 expect(host.querySelectorAll('[data-wb-enabled]').length).toBe(13);
...
test('wallboard mounts, rotates, freezes, handles failure and disposes without writes',async()=>{
 ...
 await new Promise(r=>setTimeout(r,0));for(let i=0;i<20&&host.querySelector('h1')?.textContent!=='사람별 업무';i++)host.querySelector('[data-wb="next"]').click();expect(host.textContent).toContain('가상 직원');expect(host.textContent).not.toContain('DO_NOT_DISPLAY');
