...
Current status: domain replay, research command processing, immutable repository event patches, atomic task-head patches, Firebase hydration and import validation are implemented. Node verification: 419 tests pass. The existing full Rules suite still passes for compatibility; no follow-up Rules or UI are enabled yet. The company UI and production DB do not expose this new workflow yet. Existing Windows review packages remain the previously verified integrated build until this feature is wired and rebuilt. Do not claim EVD-06 complete from domain tests alone.
