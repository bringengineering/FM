...
건물주 승인·입금 안내에 쓰는 계좌는 **소스에 넣지 않고** 스크립트 속성에서 읽습니다.
설정 전에는 계좌가 화면에 표시되지 않고, 승인 처리도 진행되지 않습니다(fail-closed).

Apps Script 편집기 → 프로젝트 설정 → 스크립트 속성에 다음을 등록하세요.
...
* 계좌번호는 숫자와 하이픈만 사용하며, 숫자 8자리 이상이어야 합니다.
* 예시용 번호(`123-456-789012`)는 등록해도 거부됩니다.
* 미설정 시 건물주 화면에는 "입금 계좌는 담당자가 확인 후 별도로 안내드립니다."가 표시됩니다.
