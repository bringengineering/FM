...
4. 온보딩 수집서 DOCX 파일이 들어 있는 Google Drive 폴더를 연결하려면 코드 상단의 `CONTRACT_DRIVE_FOLDER_ID`에 폴더 ID 또는 폴더 URL을 넣습니다.
   ...
   - ⑥ 사업자등록증은 견적서 저장 폴더 안의 케이스 폴더에 저장합니다. 구조는 `{접수번호}_{건물명} / 사업자등록증 / {업체명} / 원본 사업자등록증`입니다.
...
- ⑥ 사업자등록증은 견적서 저장 폴더의 케이스 폴더 안에 `사업자등록증 / {업체명} / 원본 사업자등록증` 구조로 저장합니다. 예: `{접수번호}_{건물명} / 사업자등록증 / {업체명} / 원본 사업자등록증`. 업체명을 못 찾으면 `업체확인필요` 폴더를 사용합니다.
- 사업자등록증 업로드 후 분석값이 같은 업체 견적 카드와 매칭되면, 기존 브링 엑셀 견적서의 빈칸을 자동으로 보충해 최신 엑셀 링크로 교체합니다. 원본 견적서와 사업자등록증 원본은 삭제하지 않습니다.
