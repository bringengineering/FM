...
현재 연결된 Realtime Database는 `BRING-FM` 프로젝트의 `https://bring-fm-default-rtdb.asia-southeast1.firebasedatabase.app`입니다. 공개 민원 호환 자료는 `cases`/`caseSettings`, 로그인 사용자 입금 자료는 `crmCompany/paymentCalendars`에 저장됩니다.
...
8. 네이버클라우드 SENS 문자 발송을 쓰려면 `프로젝트 설정` -> `스크립트 속성`에 아래 값을 추가합니다.
   ...
   - `KAKAO_TEMPLATE_CUSTOMER_QUOTE`: `BRINGCUSTOMERQUOTEV1` (CRM 고객 견적서 안내)
   - `KAKAO_TEMPLATE_COMPLETION_REPORT`: `BRINGCOMPLETIONREPORTV1` (CRM 작업 결과보고서 안내)
   - 위 두 문서 템플릿은 네이버클라우드 검수가 `완료`가 된 뒤에만 실제 발송에 사용합니다. 현재처럼 `접수` 상태일 때는 문서 발송 기능을 활성화하지 않습니다.
   - CRM `고객 메시지`는 아래 코드와 승인 본문을 한 쌍으로 설정한 템플릿만 활성화됩니다. 본문의 `#{customerName}`·`#{note}` 변수는 발송 시 CRM 값으로 치환됩니다.
   - `KAKAO_CUSTOMER_TEMPLATE_CLEANING_SCHEDULE` / `KAKAO_CUSTOMER_BODY_CLEANING_SCHEDULE`
   - `KAKAO_CUSTOMER_TEMPLATE_MOVE_IN_CLEANING` / `KAKAO_CUSTOMER_BODY_MOVE_IN_CLEANING`
   - `KAKAO_CUSTOMER_TEMPLATE_REQUESTED_FOLLOWUP` / `KAKAO_CUSTOMER_BODY_REQUESTED_FOLLOWUP`
   - `KAKAO_CUSTOMER_TEMPLATE_WORK_COMPLETED` / `KAKAO_CUSTOMER_BODY_WORK_COMPLETED`
   - `KAKAO_CUSTOMER_TEMPLATE_CLEANING_REENGAGEMENT` / `KAKAO_CUSTOMER_BODY_CLEANING_REENGAGEMENT`
   - `KAKAO_CUSTOMER_TEMPLATE_BUILDING_MANAGEMENT` / `KAKAO_CUSTOMER_BODY_BUILDING_MANAGEMENT`
   - `KAKAO_CUSTOMER_TEMPLATE_PROMOTION` / `KAKAO_CUSTOMER_BODY_PROMOTION`
   - 광고성 템플릿은 CRM의 해당 채널 수신 동의·증빙이 없으면 서버에서 차단됩니다. 알림톡 실패 시 SMS 자동 대체도 사용하지 않습니다.
...
건물주 단일 건물 월세 입금 현황용 `BRINGRENTOWNERSUMMARYV1` 템플릿은 아래 내용 그대로 등록하고 검수를 요청합니다. 현재 자동 발송은 다건물 통합 템플릿을 사용하지만, 단일 템플릿도 같은 고정 안내 문구를 유지합니다.
...
1. ⑫가 완료되어 ⑬이 `진행중`인지 확인합니다.
2. ⑬ 메모 버튼을 열고 `작업 전`과 `작업 후` 영역에 사진을 각각 선택하거나 드래그합니다.
3. JPG/JPEG, PNG, WEBP 사진을 여러 장 누적할 수 있으며 사진 한 장당 최대 5MB입니다.
4. `선택 사진 모두 업로드`를 누르면 Drive의 `{접수번호}_{건물명}/작업 사진/작업 전` 또는 `작업 후` 폴더에 원본이 저장됩니다.
5. 업로드된 사진은 ⑬에서 전·후로 나뉘어 표시되고, ⑭ B/A 보고서 참고 영역과 보고서 문서에 자동으로 연결됩니다.
6. `목록에서 제거`는 케이스 화면에서만 사진을 제외하며 감사 기록을 위해 Drive 원본은 삭제하지 않습니다.
7. 사진 확인이 끝나면 관리자가 ⑬을 완료해 ⑭를 진행합니다.
