...
- ⑧ 발송용 추천 견적 이미지는 케이스 폴더의 `건물주 추천 발송` 하위에 `4:5` 세로형 JPG로 저장합니다. `owner-summary-v2` 디자인은 최종 합계금액을 가장 크게 표시하고 추천 업체, 방문 가능 시간, 작업 품목 최대 4개, 공급가액, 부가세만 남깁니다. 구버전 표 이미지는 새 발송에서 재사용하지 않습니다. SENS 발송 요청이 접수된 뒤 실제 문자 수신을 확인하고 `발송 완료 확인`을 눌렀을 때 ⑧은 완료, ⑨ 승인·입금은 진행중으로 자동 전환합니다. 연락처 없음, 이미지 생성 실패, SENS 오류는 ⑧ 진행중/보류로 남깁니다.
