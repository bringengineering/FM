...
4. 온보딩 수집서 DOCX 파일이 들어 있는 Google Drive 폴더를 연결하려면 코드 상단의 `CONTRACT_DRIVE_FOLDER_ID`에 폴더 ID 또는 폴더 URL을 넣습니다.
   ...
   - ⑥ 사업자등록증은 `BUSINESS_REGISTRATION_DRIVE_FOLDER_ID`가 있으면 해당 폴더에 저장하고, 비어 있으면 견적서 저장 폴더 아래 `사업자등록증` 폴더를 자동 생성해 업체명 기준으로 분리 저장합니다.
...
- ⑥ 사업자등록증은 견적서 케이스 폴더 안에 섞지 않고 `사업자등록증 / {업체명} / 원본 사업자등록증` 구조로 분리 저장합니다. 업체명을 못 찾으면 `업체확인필요` 폴더를 사용하며, 별도 Drive 폴더를 쓰려면 `BUSINESS_REGISTRATION_DRIVE_FOLDER_ID`에 그 폴더 ID를 넣으면 됩니다.
...
- 브링 양식 생성 직후에는 Google Sheet 셀 `D15 -> J30 -> E30+H30` 순서로 합계금액을 다시 읽어 견적 카드에 `브링 양식 기준` 금액으로 표시합니다.
