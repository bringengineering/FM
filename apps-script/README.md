...
FM 입금확인 캘린더를 열면 `syncPopbillBankTransactions`가 로그인 사용자의 Firebase ID 토큰으로 전용 경로에 거래를 저장합니다. 화면이 열려 있는 동안 30분 간격으로 다시 조회하며, 정확히 일치하는 입금자명·금액·납부기간은 `입금완료`, 중복 후보는 `확인필요`로 표시합니다. Firebase에는 입금일·입금자명·금액·계좌번호 끝 4자리만 저장하며 계좌 비밀번호와 조회전용 ID/PW는 저장하지 않습니다.
