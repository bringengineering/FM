...
- ⑧ 발송용 추천 견적 이미지는 케이스 폴더의 `건물주 추천 발송` 하위에 핵심 요약형 세로 JPG로 저장합니다. 추천 업체, 작업 내용, 방문 가능 시간, 공급가액, 부가세, 최종 합계금액만 표시하며 최종 합계금액을 크게 강조합니다. SENS 발송 요청이 접수된 뒤 실제 문자 수신을 확인하고 `발송 완료 확인`을 눌렀을 때 ⑧은 완료, ⑨ 승인·입금은 진행중으로 자동 전환합니다. 연락처 없음, 이미지 생성 실패, SENS 오류는 ⑧ 진행중/보류로 남깁니다.
