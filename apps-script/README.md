...
# 브링FM 프로젝트 현황표 자동화 (우선순위 + To-Do + 대시보드)

이 폴더의 `project-status-dashboard.gs`는 **프로젝트 현황표**에서 과업만 입력하면
...
1. 위 현황표 Google Sheets를 엽니다.
2. 상단 메뉴 `확장 프로그램` → `Apps Script`를 엽니다.
3. 기본 코드를 지우고 `project-status-dashboard.gs` 내용을 붙여넣고 저장합니다.
4. 함수 선택 드롭다운에서 `setupProjectStatus`를 선택하고 `실행`을 누릅니다.
   - 처음 실행 시 권한 승인(내 계정으로) 창이 뜨면 허용합니다.
5. 실행이 끝나면 시트로 돌아가 새로고침합니다. `대시보드`가 맨 앞(시트1)에 생깁니다.
6. 이후에는 시트 상단에 생기는 `📊 프로젝트 현황` 메뉴에서 다시 만들거나 예시 데이터를 채울 수 있습니다.

## 사용법

- **입력은 `과업입력` 탭에서만** 합니다. 흰색 칸(과업명·담당자·카테고리·중요도·긴급도·마감일·공수·상태·진척률·비고)만 채우면 됩니다.
- 노란색 칸(우선순위점수·등급·D-Day)과 `우선순위ToDo`, `대시보드`는 **자동 계산**됩니다.
- 담당자·카테고리·진행상태는 드롭다운으로 선택합니다. 목록은 `설정` 탭에서 바꿀 수 있습니다.

## 우선순위 산정 방식

```
우선순위점수 = 중요도×3 + 긴급도×3 + 마감가중(최대 15) − 진척감점(최대 5)
  · 마감가중: 마감이 가까울수록/지났을수록 높음 (당일·초과 = 15, 15일 이상 남으면 0)
  · 진척감점: 진척률이 높을수록 점수를 조금 낮춤 (이미 진행된 일 우선순위 하향)
  · 진행상태가 '완료'면 점수 0

등급  🔴 P1 ≥ 30   ·   🟡 P2 ≥ 18   ·   🟢 P3 < 18   ·   ✅완료
```

## 대시보드 지표

- **정량**: 총/완료/진행중/대기/보류 과업 수, 완료율, 평균 진척률, 지연 건수,
  총·완료·잔여 공수, P1/P2/P3 개수, 오늘 마감 수, 카테고리별·담당자별 현황
- **정성**: 한 줄 브리핑, 이번 주 최우선 Top3, 지연 리스크, 오늘/내일 마감, 메모 모아보기

주차 표기(`WEEK_LABEL`)와 카테고리/담당자 목록은 코드 상단 상수 또는 `설정` 탭에서 조정합니다.
