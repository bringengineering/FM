...
4. 온보딩 수집서 DOCX 파일이 들어 있는 Google Drive 폴더를 연결하려면 코드 상단의 `CONTRACT_DRIVE_FOLDER_ID`에 폴더 ID 또는 폴더 URL을 넣습니다.
   ...
   - ⑥ 사업자등록증은 견적서 저장 폴더 안의 케이스 폴더에 저장합니다. 구조는 `{접수번호}_{건물명} / 사업자등록증 / BR-..._{업체명}_사업자등록증.pdf`입니다.
...
9. 케이스 화면의 ② 문자 발송, ⑤ 업체 MMS 발송, ⑥ 파일 업로드를 쓰려면 Apps Script에서 `배포` -> `새 배포` -> `웹 앱`으로 배포합니다.
   ...
   - 배포 후 생성된 웹 앱 URL을 FM 앱 ② `SENS 문자`, ⑤ `업체 발송`, ⑥ 업로드에서 묻는 자동화 웹앱 URL에 붙여넣습니다.
...
- 케이스 화면 ② `SENS 문자` 버튼을 누르면 Apps Script 웹앱의 `sendComplaintReceiptSms` 액션으로 접수 확인 문자를 다시 요청할 수 있고, 결과는 케이스 ② 메모와 응답 시트의 문자 발송 상태/메모에 반영됩니다.
...
- ⑥ 사업자등록증은 견적서 저장 폴더의 케이스 폴더 안에 `사업자등록증 / BR-..._{업체명}_사업자등록증.pdf` 구조로 바로 저장합니다. 예: `{접수번호}_{건물명} / 사업자등록증 / BR-2026-0002_업체명_사업자등록증.pdf`. 업체명을 못 찾으면 파일명에 `업체확인필요`를 사용합니다.
