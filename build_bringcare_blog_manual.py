...
OUT = Path("manuals/브링케어_네이버블로그_마스터매뉴얼_v1.2.docx")
...
h2("권장 파일명 규칙")
add_bullets([
    "마스터 파일: 브링케어_네이버블로그_마스터매뉴얼_v1.2.docx",
...
add_table(["버전", "날짜", "변경 내용", "변경 이유", "영향 범위"], [
    ...
    ("1.2", "2026-08-29", "공식블로그 100개 표본, 5종 템플릿, 첫 화면 운영 규칙 추가", "반복 가능한 시리즈와 첫 화면 탐색 구조 강화", "기획·제목·템플릿·첫 화면·검증"),
    ("", "", "", "", ""), ("", "", "", "", ""),
...
chapter(30, "공식블로그 100개 표본과 첫 화면 운영", "네이버 블로그팀 공개 게시물 100개에서 반복 가능한 구조만 추출해 브링케어에 맞게 사용한다.")
h2("30.1 표본에서 확인한 패턴")
add_table(["패턴", "100개 중 확인", "브링케어 적용"], [
    ("시리즈·대괄호 접두어", "35", "반복 가능한 포맷에만 고정 라벨"),
    ("이모지", "36", "가이드·월간 기록에 제한 사용"),
    ("질문형", "7", "독자의 실제 질문일 때만 사용"),
    ("느낌표", "83", "공식 이벤트 문법을 그대로 복제하지 않음"),
    ("숫자 표현", "66", "본문·현장 기록으로 증명될 때만 사용"),
    ("월·날짜·연도", "49", "일정·회차·월간 기록을 명확히 표시"),
    ("행동 표현", "60", "제목의 약속을 본문에서 실제로 완결"),
], widths=[1.7, 1.15, 3.85], font_size=8.3)
add_callout("해석 주의", "댓글 표시 85건의 중앙값은 371개였지만 공식 채널 규모와 경품 이벤트 효과가 포함되어 있습니다. 브링케어 목표치나 일반 블로그 벤치마크로 사용하지 않습니다.", fill=YELLOW, accent=NAVY)
h2("30.2 브링케어 권장 시리즈")
add_para("권장 시리즈는 현장 BEFORE → 확인 → 완료, 건물주 3분 가이드, 퇴실 14일 전, 이번 달 관리 기록입니다.")
add_table(["시리즈", "사용 조건", "핵심 구조"], [
    ("현장 BEFORE → 확인 → 완료", "실제 사진과 단계 기록이 모두 있음", "상태·판단·역할·결과"),
    ("건물주 3분 가이드", "한 가지 관리 질문을 쉽게 설명", "상황·기준 3개·기록·행동"),
    ("퇴실 14일 전", "일정과 예외 조건이 확인됨", "D-14·당일·다음 입주"),
    ("이번 달 관리 기록", "공개 가능한 실제 활동을 묶음", "월간 활동·배운 점·다음 기준"),
], widths=[2.0, 2.25, 2.45], font_size=8.1)
h2("30.3 글 유형별 5종 템플릿")
add_bullets(["검색 해결형: 공식 근거와 안전한 행동 순서", "현장 기록형: 실제 사진과 직접·외부·승인 역할 구분", "건물주 가이드형: 어려운 관리 용어를 생활 언어로 번역", "입퇴실형: D-14부터 다음 입주까지의 일정과 예외", "회사 공지형: 회사 정의·서비스·진행 방식·가격·실제 활동·상담을 한 글에서 이해"])
h2("30.4 첫 화면 구성")
add_numbers(["한 문장 정체성: 건물의 일을, 한 곳에서 끝냅니다", "핵심 메뉴: 브링케어 안내 / 입퇴실 24H / 현장 기록 / 건물주 가이드", "대표 공지 3개", "실제 현장 기록 3개", "건물주 가이드 2개", "카카오 상담 CTA 1개"])
add_para("첫 화면은 최신 글 한 편을 크게 노출하는 일기형 구성이 아니라, 처음 온 건물주가 회사·서비스·근거·다음 행동을 10초 안에 고를 수 있는 안내판으로 운영한다.")
h2("30.5 제목·도입·CTA 검증")
add_bullets(["시리즈 라벨과 글 유형이 일치한다.", "숫자·기간·회차에 내부 근거가 있다.", "첫 5문장에 구체 장면·감정·예상 밖 문제·질문이 있다.", "같은 회사 소개 문단을 반복하지 않는다.", "CTA는 글의 역할에 맞는 하나뿐이다.", "현장 글의 이미지는 실제 제공 사진뿐이다."])
