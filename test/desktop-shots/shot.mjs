...
const stub = (data) => {
  const ARRAY_KEYS = /^(load|read)/;
  window.bringCRM = new Proxy({}, {
    get: (_t, key) => {
      const name = String(key);
      if (name.startsWith("on")) return () => () => {};
      if (name === "authState") return async () => ({ required: false, user: { displayName: "김관리", email: "manager@bring.co.kr" } });
      if (name === "load") return async () => data;
      if (name === "loadOperations") return async () => ({ cases: [], payments: {}, caseSettings: {}, loadedAt: new Date().toISOString() });
      if (name === "dataPath") return async () => "C:\\\\Users\\\\bring\\\\AppData\\\\Roaming\\\\bring-crm";
      if (name === "updateState") return async () => ({ status: "idle", currentVersion: "1.36.1" });
      if (ARRAY_KEYS.test(name)) return async () => [];
...
const VIEWS = process.env.VIEWS
  ...
  : ["dashboard", "officeHome", "officeAttendance", "officeMessenger", "customers", "partnerVendors", "vacancies", "customerMessages", "buildingCalendar", "payments", "valueScope", "aiAssistant", "relationships", "cases", "security", "settings"];
...
// 이 브랜치는 '작업 폴더 선택' 화면으로 시작한다. 운영 폴더로 들어가야 CRM 이 보인다.
const landing = page.locator('.workspace-folder-card[data-workspace-enter="operations"]');
if (await landing.count()) {
  await page.screenshot({ path: path.join(OUT, "00-landing.png") });
  console.log("찍음: 00-landing");
  await landing.first().click();
