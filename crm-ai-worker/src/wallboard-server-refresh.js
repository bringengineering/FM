...
export async function refreshWallboardFromFirebase({idToken,identity,env,fetchImpl=fetch,now=Date.now}){
 ...
 const namedProjects=rows(source.projects).map(item=>({...item,owner:safeName(item.owner)}));
 ...
 for(let attempt=0;attempt<2;attempt++){
  ...
  const snapshot=validatePublication({model,playlist:current.presentation?.playlist||defaultPlaylist,notice:current.presentation?.notice||'',dataDate});
  if(JSON.stringify(snapshot).length>65536)fail('WALLBOARD_UNAVAILABLE');
  try{const result=await command(stub,'publish',{snapshot,expectedVersion:current.version});return {version:result.version,publishedAt:result.publishedAt};}
