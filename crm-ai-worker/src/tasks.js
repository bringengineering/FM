...
const TASKS = Object.freeze({
  ...
  directive_draft: {
    instruction: "대표가 대충 적은 이번 주 할 일을 주간 업무지시서 초안으로 바꾸세요. 반드시 아래 모양의 탭으로 갈라진 글만 쓰고, 인사말이나 설명은 한 줄도 붙이지 마세요.\n배경\\t(왜 하필 이번 주에 이것인지)\n목표\\t(이 주가 끝나면 무엇이 달라져 있는지)\n안 하면\\t(안 했을 때 무엇을 잃는지)\n제외\\t(이번 주에 하지 않는 것)\n(빈 줄)\n업무명\\t목적\\t완료기준\\t산출물\\t산출물 종류\\t산출물 수량\\t예상시간\\t가중치\\t마감\n그 아래에 업무를 한 줄씩. 규칙은 이렇습니다. 적히지 않은 사실을 만들지 마세요 — 마감일이 안 적혀 있으면 그 칸을 비우고, 없는 건물명·금액·사람 이름을 지어내지 마세요. 완료기준은 '무엇이 있으면 끝인지'를 눈에 보이는 것으로 쓰세요('열심히 한다' 같은 말은 완료기준이 아닙니다). 산출물은 YYYYMMDD_이름.확장자 모양의 파일명으로 쓰세요. 산출물 종류는 사진·문서·표·링크·없음 중 원문에서 확인한 것만 쓰고, 종류나 수량을 알 수 없으면 비워 두세요. 비어 있는 항목은 사람이 확인하기 전 발행되지 않습니다. 예상시간은 0.5 단위 숫자만 쓰고, 주어진 가용시간이 있으면 합이 그것을 넘지 않게 하세요. 가중치는 정수이고 합이 정확히 100이어야 합니다. 업무는 3~7건으로 나누세요.",
